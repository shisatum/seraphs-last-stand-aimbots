# Fullscreen overlay test using OpenGL
# src: https://guidedhacking.com/threads/fullscreen-opengl-overlay-with-python-ogl-glfw.16296/
#   says to pip install pypiwin32 pywin32 pyopengl pyopengl-accelerate
# had to install:
#   pip install glfw
# had to install from WHL:
# https://python.tutorialink.com/pyopengl-opengl-error-nullfunctionerror-attempt-to-call-an-undefined-function-glutinit-check-for-boolglutinit-before-calling/
# Download packages from: https://www.lfd.uci.edu/~gohlke/pythonlibs/#pyopengl then cd to downloads:
...
with FullScreenOverlay() as overlay:
    ...
    while overlay.loop:
        counter += 1
        if counter > 60: # Show overlay for one second
            #overlay.close_window()
            print("Would have closed window here. Manually quitting instead")
            quit()
